<template>
  <header class="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 shadow-lg">
    <nav class="container mx-auto px-4 py-4">
      <div class="flex justify-between items-center">
        <NuxtLink to="/" class="text-2xl font-bold text-white hover:text-gray-100 transition-colors">
          🛒 Маркетплейс
        </NuxtLink>

        <div class="flex items-center gap-6">
          <NuxtLink to="/" class="text-white hover:text-gray-100 font-medium transition-colors">
            Товары
          </NuxtLink>

          <template v-if="isAuthenticated">
            <NuxtLink to="/cart" class="relative text-white hover:text-gray-100 transition-colors">
          ...
          <template v-else>
            <NuxtLink to="/login">
              <UButton variant="ghost" class="text-white hover:bg-white/20">Войти</UButton>
            </NuxtLink>
            <NuxtLink to="/register">
              <UButton class="bg-white text-purple-600 hover:bg-gray-100">Регистрация</UButton>
            </NuxtLink>
          </template>
        </div>
      </div>
