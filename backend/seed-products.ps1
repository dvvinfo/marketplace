...
$products = @(
    ...
    @{
        ...
        imageUrl = "https://images.unsplash.com/photo-1696446702183-cbd50c6e8e0f?w=400"
    },
    @{
        name = "MacBook Pro 16"
...
foreach ($product in $products) {
    try {
        ...
        Write-Host "  Created product: $($product.name) - $($product.price) RUB" -ForegroundColor Green
    }
    catch {
        Write-Host "  Error creating product: $($product.name)" -ForegroundColor Yellow
    }
}
